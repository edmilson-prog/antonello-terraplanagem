...
import { dataReferenciaOperacional, serieDiariaHoras } from "@/features/dashboard-operacional/derivacoes";
...
export function CardHorasApontadas() {
  ...
  const serie = useMemo(() => serieDiariaHoras(apontamentos, referencia), [apontamentos, referencia]);
  ...
  return (
    <div className="rounded-xl border bg-card p-5 shadow-sm">
      <span className="font-mono text-[11px] uppercase tracking-wide text-foreground-faint">Horas apontadas</span>
      <div className="mt-2 font-mono text-2xl font-bold text-card-foreground">{numero.format(horas)} h</div>
      <p className="text-xs text-muted-foreground">no mês</p>
      <div className="mt-3">
        <MiniSparkline dados={serieVisual} formatar={(v) => `${numero.format(v)} h`} mostrarTooltip={false} />
      </div>
