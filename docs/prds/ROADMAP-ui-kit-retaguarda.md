...
| 30 | `DashboardOperacional.jsx` | `admin.index.tsx` (aba "Operacional") | 🔧 | Fora do escopo da Onda 11 por decisão do usuário — próxima da fila |
...
| Status | Quantidade |
|--------|------------|
| ✅ Refatorado (bate com o design system) | 25 / 34 |
| 🔧 Funcional, visual antigo | 6 / 34 |
...
| 11 | Sem plano formal — tela lida direto do projeto Claude Design via `DesignSync` (que passou a ser a fonte da verdade de design), sem etapa de Artifact | Dashboard (aba "Visão geral") | branch `worktree-dashboard-retaguarda-designsync` |

## O que falta, em ordem sugerida

1. **🔧 Páginas de área ainda sem refatoração** (6 telas: Manutenção, Diesel, Custo da Hora, Rentabilidade, Painel Gerencial, Painel Operacional) — candidatas a agrupar em ondas por afinidade, ex.: "Diesel + Manutenção" (Frota), "Custo da Hora/Rentabilidade/Painel Gerencial" (Analítico). O Painel Operacional é o par natural do Dashboard já refeito na Onda 11.
