...
> **🔧 PATCH v2 — pós-implementação (Jun/2026, revisão do Arquiteto).**
> Este PRD **já está implementado** (`_DONE`); o documento foi alterado **depois** da implementação e **o código ainda não reflete o delta abaixo**.
>
> **Delta:** Contrato `IApontamento` ganhou `modalidade: 'seca' | 'operada' | null` e `metros_executados: number | null`; mock `obras.ts` marcado como temporário (migração para `ordens-servico.ts`, PRD-003); 2 novas Decisões Pendentes.
>
> **Ação no código:** Adicionar os dois campos ao type e aos mocks; capturar `modalidade` ao **iniciar** o apontamento e `metros_executados` ao **finalizar** quando a OS for `por_metro`. Se `obras.ts` ainda existir no código, concluir a migração para `ordens-servico.ts`.
>
> **Instruções:** aplicar **somente o delta** — não reimplementar o PRD. Ao mesclar este arquivo no repositório, **preservar o Status de Implementação real** (data/versão/observações registrados na implementação original). Os deltas com código dos PRDs 002/003/004/012 formam **um patch consolidado**: versionar como **MINOR** (codinome sugerido: **"Retrofit"**), registrar no `CHANGELOG.md` (Added/Changed) e atualizar o INDEX ao concluir.
>
> **✅ Delta aplicado em 2026-07-02 (v0.11.0 "Retrofit").**
...
interface IApontamento {
  ...
  modalidade: 'seca' | 'operada' | null  // ⚠️ base do cálculo do PRD-004. Localização em aberto:
                                         // hipótese = por apontamento; pode migrar p/ OS/equipamento (ver Decisões)
  metros_executados: number | null       // p/ OS por_metro: metragem apontada — append-only,
                                         // somada na OS como as horas (coerente c/ hipótese do spike PRD-000)
...
| `src/mocks/obras.ts` (auxiliar) | ~3 OS/obras mínimas para o seletor opcional | **Mock temporário — dívida planejada:** será substituído por `ordens-servico.ts` no PRD-003 (atualizar imports e remover `obras.ts` naquele momento) |
...
- [ ] **Modalidade seca×operada:** o campo nasce no apontamento (hipótese do contrato), mas pode migrar para a OS ou o equipamento — confirmar com Leonardo (impacta o cálculo do PRD-004).
- [ ] **Metragem (modelo por_metro):** confirmado que é o **operador** quem aponta os metros? (hipótese append-only, a validar no spike PRD-000)
...
| Jul/2026 | v3 | ✅ Delta de código aplicado (v0.11.0 "Retrofit") |
