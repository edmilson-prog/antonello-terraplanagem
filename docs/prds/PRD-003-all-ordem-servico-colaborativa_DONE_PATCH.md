...
>
> **Delta:** `metragem_executada` **removida do cabeçalho** de `IOrdemServico` — passa a ser **derivada** (soma de `apontamentos.metros_executados`, PRD-002), como as horas; RF-015 e mock ajustados. Motivo: metragem no cabeçalho criaria o conflito multi-operador que a hipótese do spike (PRD-000) quer evitar.
>
> **Ação no código:** Remover o campo do type, dos mocks e do formulário do cabeçalho; exibir a metragem **derivada** no detalhe da OS (mesmo padrão do total de horas).
>
...
- **RF-015 (Must):** No modelo `por_metro`, a OS registra o **diâmetro da broca** no cabeçalho; a **metragem executada é derivada** da soma dos `metros_executados` dos apontamentos vinculados (PRD-002), exatamente como as horas.
...
interface IOrdemServico {
  ...
  // ⚠️ metragem_executada é DERIVADA — soma de apontamentos.metros_executados (PRD-002),
  // como as horas. Não vive no cabeçalho: metragem no cabeçalho criaria exatamente o
  // conflito multi-operador que a hipótese do spike (PRD-000) quer evitar.
  // ciclo:
...
|--------------|----------|----------------------|
| `src/mocks/ordens-servico.ts` | ~6 OS (mix de `hora_maquina` e `por_metro`) | 1 `aberta` sem apontamentos, 1 `em_andamento` colaborativa (apontamentos de 2+ operadores), 1 `fechada`, 1 com `pendente_sync: true`, 1 modelo `por_metro` com diâmetro no cabeçalho e metragem nos apontamentos vinculados |
