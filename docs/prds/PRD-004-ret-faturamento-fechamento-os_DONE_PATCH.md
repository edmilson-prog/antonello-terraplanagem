...
>
> **Delta:** Orientação de cálculo: a tarifa **seca × operada** é escolhida lendo `modalidade` **do apontamento** (PRD-002, contrato provisório); Decisão Pendente atualizada.
>
> **Ação no código:** Ajustar a montagem dos itens do faturamento para agrupar horas por **equipamento + modalidade** e aplicar a tarifa correspondente. Implementar a leitura da modalidade de forma **trocável** (a localização final do campo é decisão aberta com o Leonardo).
>
...
- [ ] Onde se define **seca × operada**? O contrato provisório coloca `modalidade` **no apontamento** (PRD-002) — confirmar com Leonardo se permanece lá ou migra para a OS/equipamento.
...
| **Modalidade (seca×operada)** | Ler `modalidade` do apontamento (PRD-002, contrato provisório) para escolher a tarifa; a localização final do campo é decisão aberta — implementar de forma que a origem seja fácil de trocar |
